...
class NumberedMargin(Margin):
    ...
    def get_width(self, get_ui_content):
        ...
        return max(3, len('%s' % line_count) + 2)
    ...
    def create_margin(self, window_render_info, width, height):
        ...
        for y, lineno in enumerate(window_render_info.displayed_lines):
            # Continuation of previous line (wrapped line)
            if lineno == last_lineno:
                result.append((style + ' bold', '↳ '.rjust(width)))
            else:
                if lineno is None:
                    pass
...
class ScrollbarMargin(Margin):
    ...
    def __init__(self, display_arrows=False, up_arrow_symbol='^', down_arrow_symbol='v'):
        self.display_arrows = lambda: False  # Patched by Leistungsabfall
