    ...
    def mouse_handler(self, mouse_event):
        ...
        if self._fragments:
            ...
            try:
                ...
                count = -1  # Patched by Leistungsabfall: 0 -> -1 to fix menu index
        ...
        if get_app().layout.current_control == self:
            if self._last_get_processed_line:
                ...
                if mouse_event.event_type == MouseEventType.MOUSE_DOWN:
                    ...
                    buffer.cursor_position = index
                    self._old_cursor_position = buffer.cursor_position
                    ...
                    if self._old_cursor_position == buffer.cursor_position:
                        buffer.exit_selection()
                    ...
                    if double_click:
                        # Patched by Leistungsabfall: Double click NumberedMargin => select whole line
                        if position.x == -1:
                            start = buffer.document.get_start_of_line_position()
                            end = buffer.document.get_end_of_line_position()
                        else:
