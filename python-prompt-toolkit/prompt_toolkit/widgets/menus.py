...
class MenuContainer(object):
    ...
    def __init__(self, body, menu_items=None, floats=None, key_bindings=None):
        ...
        @kb.add('escape', filter=in_main_menu)  # new
        @kb.add('backspace', filter=in_main_menu)  # new
        @kb.add('c-c', filter=in_main_menu)
        @kb.add('c-g', filter=in_main_menu)
        def _(event):
            " Leave menu. "
            from globals import Globals  # this is really ugly :(
            event.app.layout.focus(Globals.text_field)

        # Sub menu navigation.

        # Patched by Leistungsabfall
        @kb.add('escape', filter=in_sub_menu)  # new
        @kb.add('backspace', filter=in_sub_menu)  # new
    ...
    def _get_menu_fragments(self):
        ...
        def one_item(i, item):
            def mouse_handler(mouse_event):
                if mouse_event.event_type == MouseEventType.MOUSE_DOWN:  # Patched by Leistungsabfall: MOUSE_UP -> MOUSE_DOWN
                    # Toggle focus.
                    app = get_app()
            ...
            # Patched by Leistungsabfall: Removed space in favor of preceding space in main menu (cosmetic)
                    ...
                    def one_item(i, item):
                        ...
                        if item.text == '-':
                            yield (style + 'class:menu-border',
                                   '{}'.format(Border.HORIZONTAL * (menu.width + 4)),  # Patched by Leistungsabfall: 3 -> 4
                                   mouse_handler)
                        else:
                            # Patched by Leistungsabfall
                            # add prefix to indicate active selection in monochrome mode
                            prefix = ' '
                            if i == selected_item:
                                if '\xb7'.encode(get_app().output.encoding(), 'replace') == b'?':
                                    ...
                                    prefix = '\xb7'
                            yield (style, ' {}'.format(prefix + item.text).ljust(menu.width + 3),
                                   mouse_handler)

                        if item.children:
                            yield (style, '>', mouse_handler)
                        elif item.text != '-':  # Patched by Leistungsabfall
                            yield (style, ' ', mouse_handler)
