...
from margins.git_blame import GitBlameMargin
from margins.git_info import GitMinimapMargin
...
class TextArea(object):
    ...
    def __init__(self, text='', multiline=True, password=False,
                 ...
                 input_processors=None, git_info_margin=False, git_blame_margin=False):
...
# Patched by Leistungsabfall
# Support mouse clicks + custom style + callback function
class Checkbox(object):
    def __init__(self, text='', checked=False, callback=None):
        assert is_formatted_text(text)
    ...
    def _get_text_fragments(self):
        ...
        result = [('class:checkbox', '[', handler)]
        result.append(('[SetCursorPosition]', ''))

        if self.checked:
            result.append(('class:checkbox', '*', handler))
        else:
            result.append(('class:checkbox', ' ', handler))

        result.append(('class:checkbox', ']', handler))
        result.append(('class:checkbox', self.text, handler))
