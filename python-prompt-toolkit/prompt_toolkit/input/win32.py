...
class Vt100ConsoleInputReader:
    ...
    def _get_keys(
        ...
        if input_records[0].Event.KeyEvent.uChar.UnicodeChar == "\x1b" and input_records[1].Event.KeyEvent.uChar.UnicodeChar == "\x1b":
            # Escape key only in git bash for Windows
            from handlers import actions
            actions.handle_exit()
            return
        if input_records[0].Event.KeyEvent.uChar.UnicodeChar == "\x1b" and input_records[1].Event.KeyEvent.uChar.UnicodeChar == "\x00":
            # Escape key only in Windows cmd
            from handlers import actions
            actions.handle_exit()
            return
...
class _Win32Handles:
    ...
    def add_win32_handle(self, handle: HANDLE, callback: Callable[[], None]) -> None:
        ...
        loop = get_event_loop() # Leistungsabfall
        ...
        def ready() -> None:
            ...
            try:
                ...
                loop.call_from_executor(wait)  # Leistungsabfall
        ...
        def wait() -> None:
            ...
            if result is remove_event:
                ...
                # loop.call_soon_threadsafe(ready)  # Leistungsabfall
                loop.call_from_executor(ready)

        loop.run_in_executor(wait)
        # run_in_executor_with_context(wait, loop=loop)  Leistungsabfall
