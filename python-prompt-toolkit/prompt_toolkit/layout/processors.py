...
class HighlightSelectionProcessor(Processor):
    ...
    """
    def apply_transformation(self, transformation_input):
        def next_line_contains_selection():
            try:
                return document.selection_range_at_line(lineno+1) is not None
            except IndexError:
                return False
        ...
        selected_newline_fragment = selected_fragment + ' bold '
        ...
        if selection_at_line:
            ...
            if from_ == 0 and to == 0 and len(fragments) == 0:
                ...
                char = ''
                if len(buffer_control.buffer.copy_selection().text) > 0 and next_line_contains_selection():
                    # Append newline char to visualize selection (empty line)
                    return Transformation([(selected_newline_fragment, newline_char)])
                return Transformation([(selected_fragment, char)])
            else:
                ...
                # Patched by Leistungsabfall
                if next_line_contains_selection():
                    # Append newline char to visualize selection
                    fragments.append((selected_newline_fragment, newline_char))
    ...
    def apply_transformation(self, ti):
        ...
        for i, fragment_and_text in enumerate(fragments):
            ...
            if fragment_and_text[1] == '\t':
                ...
                # Insert tab.
                # Patched by Leistungsabfall
                result_fragments.append((fragment_and_text[0] + ' ' + style, separator1 * (count - 1)))
                result_fragments.append((fragment_and_text[0] + ' ' + style, separator2))
                pos += count
            else:
