...
from prompt_toolkit.key_binding.bindings.scroll import scroll_forward, scroll_backward, scroll_one_line_up
...
class Window(Container):
    ...
    def _write_to_screen_at_index(self, screen, mouse_handlers, write_position,
        ...
        def mouse_handler(mouse_event):
            ...
            while x >= 0:
                ...
                if get_app().current_buffer == Globals.text_field.buffer:
                    info = get_app().layout.current_window.render_info
                    first_visible_line = info.first_visible_line()
                    additional_lines = 0
                    calced_range = range(first_visible_line, first_visible_line+y-2)
                    updated_range = 0
                    for i in calced_range:
                        height = info.get_height_for_line(i)
                        additional_lines += min(height - 1, (y-2) - updated_range)
                        updated_range += height
                        if updated_range >= len(calced_range):
                            break

                    result = self.content.mouse_handler(
                        MouseEvent(position=Point(x=x, y=first_visible_line+y-2-additional_lines),
                                   event_type=mouse_event.event_type))
                else:
                    result = self.content.mouse_handler(
        ...
        mouse_handlers.set_mouse_handler_for_range(
            x_min=write_position.xpos,  # + sum(left_margin_widths), Patched by Leistungsabfall: Select text in textarea even if cursor is in NumberedMargin
            x_max=write_position.xpos + write_position.width,  # - total_margin_width, Patched by Leistungsabfall: Make area around scrollbar in text field clickable
    ...
    def _scroll_when_linewrapping(self, ui_content, width, height):
        ...
        # cursor is visible.
        # Patched by Leistungsabfall
        # old: topmost_visible = get_topmost_visible()
        topmost_visible = ui_content.line_count - 1  # new
    ...
    def _scroll_down(self):
        # Patched by Leistungsabfall
        # old code is buggy (scrolling up and down with mouse gets stuck somewhere
        # new:
        w = get_app().layout.current_window
        buffer = get_app().current_buffer
        buffer.exit_selection()

        # move cursor to last visible line if not already
        current_line_index = buffer.document.translate_index_to_position(buffer.cursor_position)[0]
        if current_line_index < w.render_info.last_visible_line():
            buffer.cursor_position = buffer.document.translate_row_col_to_index(w.render_info.last_visible_line(), 0)
        ...
        def edge_for_scrolling_multiple(): return int(count+(1/delta)/slow_factor)
        try:
            delta = time.time() - last_scroll_time
            count = edge_for_scrolling_multiple()
        except (NameError, ZeroDivisionError):
            pass

        get_app().current_buffer.auto_down(count=count)
    ...
    def _scroll_up(self):
        ...
        # move cursor to first visible line if not already
        current_line_index = buffer.document.translate_index_to_position(buffer.cursor_position)[0]
        if current_line_index > w.render_info.first_visible_line():
            buffer.cursor_position = buffer.document.translate_row_col_to_index(w.render_info.first_visible_line(), 0)
            buffer.cursor_position += buffer.document.get_start_of_line_position(after_whitespace=False)
            return
        ...
        get_app().current_buffer.auto_up(count=count)
        last_scroll_time = time.time()
        return
        # old below
        " Scroll window up. "
